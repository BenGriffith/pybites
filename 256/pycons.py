...
def get_pycon_events(data=_get_pycon_data()) -> List[PyCon]:
    ...
    events = soup.find_all("script")
    for event in events:
        event = json.loads(event.get_text())
...
def filter_pycons(pycons: List[PyCon],
    ...
    that continent.
    """
    pycon_events_filtered = []

    for pycon in pycons:
        if get_continent(pycon.country) == continent and pycon.start_date.year == year:
            pycon_events_filtered.append(pycon)

    return pycon_events_filtered


if __name__ == "__main__":
    print(len(get_pycon_events()))
    test = get_pycon_events()
    for row in test:
        print(row)
