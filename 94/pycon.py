...
def load_pycon_data(pycon_videos=pycon_videos):
    ...
    with open(pycon_videos, "rb") as file:
        while True:
            try:
                videos.append(pickle.load(file))
            except EOFError:
                break
...
def get_talks_gt_one_hour(videos):
    ...
    videos_gt_hour = []
    for video in videos:
        if 'H' in video.duration[2:4] and int(video.duration[2]) >= 1:
            videos_gt_hour.append(video)

    return videos_gt_hour
...
def get_talks_lt_twentyfour_min(videos):
    ...
    videos_lt_24 = []
    for video in videos:
        if 'H' not in video.duration[2:4] and int(video.duration[2:4]) < 24:
            videos_lt_24.append(video)
