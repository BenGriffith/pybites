...
def sysinfo_scrape(output):
    ...
    output = output.split("\n")
    output_os = output[2]
    row_index = output_os.find("OS")
    
    system_info = {}
    for i in range(1, len(output)):
        row, row_length = output[i], len(output[i])

        if i == 1:
            system_info["Name"] = row[row_index:row_length]
            continue

        key_end = row.rfind(":")
        if key_end > 0:
            key = row[row_index:key_end]
            value = row[key_end + 2:row_length]
